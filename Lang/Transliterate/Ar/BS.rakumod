...
unit class Lang::Transliterate::Ar::BS does Lang::Transliterate::Transliterator;

# Arabic British Standard (BS) Transliteration
# Based on the British Standard for Arabic transliteration
# Similar to ALA-LC but with some differences per the comparison table
...
my %base-mappings = (
    ...
    "\c[0x062C]" => 'j',      # jim (ج)
    "\c[0x062D]" => 'ḥ',      # ha (ح)
    "\c[0x062E]" => 'ẖ',      # kha (خ) - h with dot below
    "\c[0x062F]" => 'd',      # dal (د)
    "\c[0x0630]" => 'ḏ',      # dhal (ذ)
    "\c[0x0631]" => 'r',      # ra (ر)
    "\c[0x0632]" => 'z',      # zay (ز)
    ...
    # Special forms
    "\c[0x0622]" => 'ʾā',     # alif madda (آ) - BS uses ʾā
    "\c[0x0629]" => 'h',      # ta marbuta (ة) - h in pausa, t in construct
    "\c[0x0649]" => 'ā',      # alif maqsura (ى) - BS uses ā
    
    # Definite article (BS uses lowercase a and hyphen)
...
method get-reverse-mappings(--> List) {
    # Arabic British Standard reverse mappings
    return (
        ...
        'ṯ' => "\c[0x062B]",
        'ḥ' => "\c[0x062D]",
        'ẖ' => "\c[0x062E]",     # BS distinctive feature
        'ḏ' => "\c[0x0630]",
        'š' => "\c[0x0634]",
        'ṣ' => "\c[0x0635]",
        'ḍ' => "\c[0x0636]",
