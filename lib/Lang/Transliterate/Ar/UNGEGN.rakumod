...
unit class Lang::Transliterate::Ar::UNGEGN does Lang::Transliterate::Transliterator;

# Arabic UNGEGN Transliteration
# Based on the UNGEGN (United Nations Conference on the Standardization of Geographical Names) standard for Arabic
# Very similar to BGN/PCGN but with some differences
...
my %base-mappings = (
    ...
    "\c[0x062B]" => 'th',     # tha (ث)
    "\c[0x062C]" => 'j',      # jim (ج)
    "\c[0x062D]" => 'ḩ',      # ha (ح) - with cedilla per UNGEGN
    "\c[0x062E]" => 'kh',     # kha (خ)
    "\c[0x062F]" => 'd',      # dal (د)
    "\c[0x0630]" => 'dh',     # dhal (ذ)
    "\c[0x0631]" => 'r',      # ra (ر)
    ...
    "\c[0x0633]" => 's',      # sin (س)
    "\c[0x0634]" => 'sh',     # shin (ش)
    "\c[0x0635]" => 'ş',      # sad (ص) - with cedilla per UNGEGN
    "\c[0x0636]" => 'ḑ',      # dad (ض) - with cedilla per UNGEGN
    "\c[0x0637]" => 'ţ',      # ta (ط) - with cedilla per UNGEGN
    "\c[0x0638]" => 'dh',     # za (ظ) - UNGEGN uses 'dh' for ظ (different from BGN/PCGN)
    "\c[0x0639]" => 'ʻ',      # ayn (ع)
    "\c[0x063A]" => 'gh',     # ghayn (غ)
    "\c[0x0641]" => 'f',      # fa (ف)
    "\c[0x0642]" => 'q',      # qaf (ق)
    ...
    # Special forms
    "\c[0x0622]" => 'ā',      # alif madda (آ) - UNGEGN uses ā, not ʼā
    "\c[0x0629]" => 'h',      # ta marbuta (ة) - h in pausa, t in construct
    "\c[0x0649]" => 'á',      # alif maqsura (ى)
    
    # Definite article (UNGEGN uses lowercase a and hyphen like ALA-LC)
    "\c[0x0627]\c[0x0644]" => 'al-',   # definite article (ال) - lowercase with hyphen
...
method get-reverse-mappings(--> List) {
    # Arabic UNGEGN reverse mappings
    return (
        # Multi-character mappings first
        'al-' => "\c[0x0627]\c[0x0644]",
        'th' => "\c[0x062B]",
        'kh' => "\c[0x062E]",
        'dh' => "\c[0x0630]",  # Note: UNGEGN uses 'dh' for both ذ and ظ
        'sh' => "\c[0x0634]",
        'gh' => "\c[0x063A]",
        'ʼa' => "\c[0x0623]",
        'ʼi' => "\c[0x0625]",
