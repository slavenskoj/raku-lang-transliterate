...
unit class Lang::Transliterate::Fa::BGNPCGN does Lang::Transliterate::Transliterator;

# Persian BGN/PCGN Transliteration
# Based on the BGN/PCGN romanization standard from 1958
...
my %base-mappings = (
    ...
    "\c[0x062D]" => 'ḩ',      # he jimi (ح) - h with cedilla (or ḥ with dot)
    ...
    "\c[0x0635]" => 'ş',      # sad (ص) - s with cedilla (or ṣ with dot)
    "\c[0x0636]" => 'ẕ',      # zad (ض) - z with two dots below
    "\c[0x0637]" => 'ţ',      # ta (ط) - t with cedilla (or ṭ with dot)
    "\c[0x0638]" => 'z̧',      # za (ظ) - z with cedilla (or ẓ with dot)
    "\c[0x0639]" => 'ʼ',      # eyn (ع) - apostrophe (not at word beginning)
    ...
    "\c[0x0629]" => '',       # te marbuta (ة) - not used in Persian according to table
...
method get-reverse-mappings(--> List) {
    # Persian BGN/PCGN reverse mappings
    return (
